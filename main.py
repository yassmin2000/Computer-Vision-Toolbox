...
from ui_handler import handlecomboBoxChange, handlecomboBoxChange2
...
class mainwindow (QtWidgets.QMainWindow):
    ...
    def display_image(self, img_data, widget):
        ...
        if len(img_data.shape) == 2:  # Grayscale image
            img_height, img_width = img_data.shape
            bytes_per_line = img_width
            q_image = QImage(bytes(img_data.data), img_width, img_height, bytes_per_line, QImage.Format_Grayscale8)
        elif len(img_data.shape) == 3:  # Color image
            img_height, img_width, channels = img_data.shape
            bytes_per_line = 3 * img_width
            img_rgb = cv2.cvtColor(img_data, cv2.COLOR_BGR2RGB)

            q_image = QImage(img_rgb.data, img_width, img_height, bytes_per_line, QImage.Format_RGB888)
        else:
            raise ValueError("Unsupported image format.")
    ...
    def convert_gry(self, img_data):
        if len(img_data.shape) == 3:  # Check if it's a color image
            gray_image = np.zeros((img_data.shape[0], img_data.shape[1]), dtype=np.uint8)
    ...
    def convert_array_to_image(self, img_array, original_img_dtype):
        ...
        img_rgb = cv2.cvtColor(img_array, cv2.COLOR_BGR2RGB)  # Assuming img_array is in BGR format
        ...
        q_image = QImage(img_rgb.data, img_width, img_height, img_rgb.strides[0], QImage.Format_RGB888)

        return q_image
    
    def edge_detection(self, selected_method, selected_dir, low_threshold, high_threshold):
    ...
    def thresholding(self):
        if self.comboBox_2.currentIndex() == 0:
            thresholded_image_path = global_thresholding(self.original_img, self.horizontalSlider.value())
        else:
            ...
            thresholded_image_path = local_thresholding(self.original_img, t1, t2, t3, t4)
    ...
    def apply_button_clicked(self):
        if self.filters_combo.currentIndex() == 0:
            ...
            if selected_method == "Canny":
                ...
                self.edge_detection(selected_method, selected_dir, low_threshold, high_threshold)
            else:
